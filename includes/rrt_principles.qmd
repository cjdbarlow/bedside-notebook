...
:::column-margin
$TMP = {P_{Filter} + P_{Return} \over 2} - {P_{Effluent}}$, where:
...
:::column-margin
$FF = {Rate_{UF} \over Rate_{Blood}} = {Rate_{UF} \over Rate_{Blood \ Pump} \times (1 - Hct)}$, where:
...
**Effluent** production in mL/kg/hr.
	* CRRT dose should generally be **20-25mL/hr**\
		...
		* Higher doses are not associated with ↑ outcome
		* Higher doses may be used in liver failure, with little evidence
